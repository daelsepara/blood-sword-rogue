...
namespace BloodSwordRogue
{
    ...
    template <typename T>
    using StringMap = UnorderedMap<T, std::string>;

    // maps object/type to a constant string
    template <typename T>
    using ConstStrings = UnorderedMap<T, const char *>;

    // maps object/type to an integer
    template <typename T>
    using IntegerMap = UnorderedMap<T, int>;

    // search for key (using its value), return default value if not found
    template <typename T, typename R>
    T Find(UnorderedMap<T, R> &unordered_map, R key, T default_value)
    {
    ...
    template <typename T>
    T Find(ConstStrings<T> &unordered_map, const char *key)
    {
        auto result = T::NONE;
    ...
    template <typename T>
    T Find(StringMap<T> &unordered_map, std::string key)
    {
        return Find(unordered_map, key, T::NONE);
    }
    ...
    {
        return Find(vector, key) != vector.end();
    }
    ...
    template <typename T, typename R>
    bool Has(UnorderedMap<T, R> &unordered_map, T &key)
    {
    ...
    template <typename T, typename R>
    R Find(UnorderedMap<T, R> &unordered_map, T &key)
    {
        return Has(unordered_map, key) ? unordered_map[key] : R::NONE;
    }
}
