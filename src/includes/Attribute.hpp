...
namespace BloodSwordRogue::Attribute
{
    // attribute type
    enum class Type
    {
    ...
    BloodSwordRogue::ConstStrings<Attribute::Type> AssetNames = {
    ...
    BloodSwordRogue::StringMap<Attribute::Type> TypeMapping = {
        {Attribute::Type::NONE, "NONE"},
    ...
    BloodSwordRogue::StringMap<Attribute::Type> Abbreviations = {
        {Attribute::Type::NONE, "---"},
    ...
    // get attribute type
    Attribute::Type MapAttribute(std::string attribute)
    {
        return BloodSwordRogue::Find(Attribute::TypeMapping, attribute);
    }

    // get attribute type
    Attribute::Type MapAttribute(const char *attribute)
    {
        return Attribute::MapAttribute(std::string(attribute));
    }
    ...
    const int MinModifier = -5;

    const int MaxModifier = 5;
...
namespace BloodSwordRogue::Attributes
{
    // attributes list
    ...
    Attributes::List Load(nlohmann::json &data)
    {
        ...
        for (auto i = 0; i < SafeCast(data.size()); i++)
        {
            ...
            attribute.Type = !data[i]["type"].is_null() ? Attribute::MapAttribute(std::string(data[i]["type"])) : Attribute::Type::NONE;
