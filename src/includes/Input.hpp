...
namespace BloodSwordRogue::Input
{
    ...
    void WaitForNext(Graphics::Base &graphics, Scene::Base &scene)
    {
        ...
        while (true)
        {
            ...
            SDL_WaitEventTimeout(&result, BloodSwordRogue::StandardDelay);
    ...
    Controls::User WaitForInput(Graphics::Base &graphics, Graphics::Scenery scenes, Controls::Collection &controls, Controls::User input, bool blur = true, int delay = BloodSwordRogue::StandardDelay)
    {
        ...
        else if (result.type == SDL_KEYDOWN && !controls.empty())
        {
            ...
            else if (input.Current >= 0 && input.Current < SafeCast(controls.size()))
            {
                ...
                {
                    auto escape = Controls::Find(controls, Controls::MapType("BACK"));

                    auto cancel = Controls::Find(controls, Controls::MapType("CANCEL"));

                    auto exit = Controls::Find(controls, Controls::MapType("EXIT"));

                    auto confirm = Controls::Find(controls, Controls::MapType("CONFIRM"));
        ...
        else if (result.type == SDL_CONTROLLERBUTTONDOWN && result.cbutton.button == SDL_CONTROLLER_BUTTON_X && !controls.empty())
        {
            ...
            if (input.Current >= 0 && input.Current < SafeCast(controls.size()) && (controls[input.Current].Type == Controls::MapType("SCROLL UP") || controls[input.Current].Type == Controls::MapType("SCROLL DOWN")))
            {
                input.Selected = true;
            }
        }
        ...
        else if (input.Hold)
        {
            if (input.Current >= 0 && input.Current < SafeCast(controls.size()) && (controls[input.Current].Type == Controls::MapType("SCROLL UP") || controls[input.Current].Type == Controls::MapType("SCROLL DOWN")))
            {
                if (controls[input.Current].Type == Controls::MapType("SCROLL UP"))
                {
                    input.Up = true;
                }
                else if (controls[input.Current].Type == Controls::MapType("SCROLL DOWN"))
                {
                    input.Down = true;
                }
    ...
    Controls::User RogueInput(Graphics::Base &graphics, Graphics::Scenery scenes, int delay = BloodSwordRogue::StandardDelay)
    {
        ...
        else if (result.type == SDL_KEYDOWN)
        {
            if (result.key.keysym.sym == SDLK_LEFT || result.key.keysym.sym == SDLK_KP_4)
            {
                input.Type = Controls::MapType("LEFT");
            }
            else if (result.key.keysym.sym == SDLK_RIGHT || result.key.keysym.sym == SDLK_KP_6)
            {
                input.Type = Controls::MapType("RIGHT");
            }
            else if (result.key.keysym.sym == SDLK_UP || result.key.keysym.sym == SDLK_KP_8)
            {
                input.Type = Controls::MapType("UP");
            }
            else if (result.key.keysym.sym == SDLK_DOWN || result.key.keysym.sym == SDLK_KP_2)
            {
                input.Type = Controls::MapType("DOWN");
            }
            ...
            {
                input.Type = Controls::MapType("ACTION");
            }
            else if (result.key.keysym.sym == SDLK_ESCAPE)
            {
                input.Type = Controls::MapType("MENU");
            }
            else if (result.key.keysym.sym == SDLK_m)
            {
                input.Type = Controls::MapType("MAP");
            }
            else if (result.key.keysym.sym == SDLK_q)
            {
                input.Type = Controls::MapType("EXIT");
            }
            else if (result.key.keysym.sym == SDLK_i)
            {
                input.Type = Controls::MapType("INVENTORY");
            }
            else if (result.key.keysym.sym == SDLK_h)
            {
                input.Type = Controls::MapType("HINTS");
            }
            else if (result.key.keysym.sym == SDLK_p)
            {
                input.Type = Controls::MapType("PARTY");
            }
            else if (result.key.keysym.sym == SDLK_o)
            {
                input.Type = Controls::MapType("BATTLE ORDER");
            }
        }
    ...
    Controls::User WaitForInput(Graphics::Base &graphics, Scene::Base &background, Scene::Base &scene, Controls::User input, bool is_dialog = false, bool blur = true, int delay = BloodSwordRogue::StandardDelay)
    {
        auto &controls = is_dialog ? scene.Controls : background.Controls;
    ...
    bool IsPlayer(Controls::User &input)
    {
        ...
        if (input.Type == Controls::MapType("WARRIOR"))
        {
            result = true;
        }
        else if (input.Type == Controls::MapType("TRICKSTER"))
        {
            result = true;
        }
        else if (input.Type == Controls::MapType("SAGE"))
        {
            result = true;
        }
        else if (input.Type == Controls::MapType("ENCHANTER"))
        {
            result = true;
        }
    ...
    {
        return input.Type == Controls::MapType("ENEMY");
    }
    ...
    {
        return (input.Selected && (input.Type != Controls::NONE) && !input.Hold);
    }
    ...
    {
        return (input.Type == Controls::MapType("SCROLL UP") || input.Up);
    }

    // check if input is down or scroll down
    bool IsDown(Controls::User &input)
    {
        return (input.Type == Controls::MapType("SCROLL DOWN") || input.Down);
    }
}
