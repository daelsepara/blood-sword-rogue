...
namespace BloodSwordRogue::Interface
{
    ...
    // render icon grid
    Scene::Base IconGrid(Graphics::Base &graphics, Asset::List &assets, int width, int height, Point box, Uint32 border_color, int offset)
    {
        auto scene = Scene::Base();
    ...
    void IconControls(Graphics::Base &graphics, Scene::Base &scene, Asset::List &assets, int width, int height, Point box, int offset = 0)
    {
        if (assets.empty())
        {
            return;
    ...
    Strings GetTextList(Graphics::Base &graphics, Graphics::Scenery scenes, Strings &seed, int width, int height, std::string prompt = std::string())
    {
        ...
        while (!done)
        {
            ...
            // add button
            auto add_id = SafeCast(scene.Controls.size());

            auto add = Point(box.X + controls * x_offset + BloodSwordRogue::Pad, box.Y + height - BloodSwordRogue::Pad - BloodSwordRogue::TileSize);

            scene.Add(Scene::Element(Asset::Get(Asset::Map("IDENTIFY")), add));

            scene.Add(Controls::Base(Controls::MapType("ADD"), add_id, controls > 0 ? add_id - 1 : add_id, add_id, add_id - (controls + 1), add_id, add.X, add.Y, BloodSwordRogue::TileSize, BloodSwordRogue::TileSize, Color::Active));

            controls++;

            if (items > 0)
            {
                auto remove_id = SafeCast(scene.Controls.size());

                auto remove = Point(box.X + controls * x_offset + BloodSwordRogue::Pad, box.Y + height - BloodSwordRogue::Pad - BloodSwordRogue::TileSize);

                scene.Add(Scene::Element(Asset::Get(Asset::Map("CROSS MARK")), remove));

                scene.Add(Controls::Base(Controls::MapType("REMOVE"), remove_id, controls > 0 ? remove_id - 1 : remove_id, remove_id, remove_id - (controls + 1), remove_id, remove.X, remove.Y, BloodSwordRogue::TileSize, BloodSwordRogue::TileSize, Color::Active));

                controls++;
            }

            // ok/confirm button
            auto ok_id = SafeCast(scene.Controls.size());

            auto ok = Point(box.X + controls * x_offset + BloodSwordRogue::Pad, box.Y + height - BloodSwordRogue::Pad - BloodSwordRogue::TileSize);

            scene.Add(Scene::Element(Asset::Get(Asset::Map("CONFIRM")), ok));

            scene.Add(Controls::Base(Controls::MapType("CONFIRM"), ok_id, controls > 0 ? ok_id - 1 : ok_id, ok_id, ok_id - (controls + 1), ok_id, ok.X, ok.Y, BloodSwordRogue::TileSize, BloodSwordRogue::TileSize, Color::Active));
            ...
            if (Input::Check(input))
            {
                ...
                else if (input.Type == Controls::MapType("ADD"))
                {
                    if (selected >= 0 && selected < items && items > 0)
                    {
                        ...
                        if (!text.empty())
                        {
                            text_list[selected] = std::string(text);
                        }
                    }
                    else
                    {
                ...
                else if (input.Type == Controls::MapType("REMOVE"))
                {
                    if (selected >= 0 && selected < items && items > 0)
                    {
                        text_list.erase(text_list.begin() + selected);
    ...
    void RenderValue(Scene::Base &scene, Asset::List &numbers, Asset::Type asset, int max_digits, int score, int x, int y)
    {
        ...
        for (auto digits = 0; digits < max_digits; digits++)
        {
            scene.Add(Scene::Element(x + BloodSwordRogue::TileSize * (digits + 1) + 4, y + 4, BloodSwordRogue::TileSize - 8, BloodSwordRogue::TileSize - 8, Color::Transparent, color, 2));

            auto divisor = SafeCast(std::pow(10, max_digits - digits - 1));

            scene.VerifyAndAdd(Scene::Element(Asset::Get(numbers[std::abs(temp_score / divisor)]), Point(x + BloodSwordRogue::TileSize * (digits + 1), y)));
    ...
    void RenderValue(Scene::Base &scene, Asset::List &numbers, Asset::Type asset, int max_digits, int score, int x, int y, std::string inc, std::string dec)
    {
        ...
        scene.Add(Controls::Base(Controls::MapType(inc), inc_id, inc_id, inc_id + 1, inc_id, inc_id, x + BloodSwordRogue::TileSize * (max_digits + 1), y, BloodSwordRogue::TileSize, BloodSwordRogue::TileSize, Color::Active));

        scene.VerifyAndAdd(Scene::Element(Asset::Get(Asset::Map("DOWN")), Point(x + BloodSwordRogue::TileSize * (max_digits + 2), y)));

        auto dec_id = SafeCast(scene.Controls.size());

        scene.Add(Controls::Base(Controls::MapType(dec), dec_id, dec_id - 1, dec_id, dec_id, dec_id, x + BloodSwordRogue::TileSize * (max_digits + 2), y, BloodSwordRogue::TileSize, BloodSwordRogue::TileSize, Color::Active));
    }
    ...
    int SetValue(Graphics::Base &graphics, Graphics::Scenery &scenery, Asset::List &numbers, std::string asset, int max_digits, int value, int min_value, int max_value)
    {
        ...
        while (!done)
        {
            ...
            scene.Controls.back().Right = id;

            auto confirm = Point(box.X + tile * (max_digits + 3) + tile / 2, box.Y + tile / 2);
            ...
            auto back = Point(box.X + tile * (max_digits + 4) + tile / 2, box.Y + tile / 2);
    ...
    // generate asset and captions list for an inventory
    void GenerateAssets(Items::Inventory &items, Asset::List &assets, Strings &captions)
    {
        assets.clear();
    ...
    int SelectItem(Graphics::Base &graphics, Graphics::Scenery scenes, Items::Inventory &items)
    {
        ...
        Interface::GenerateAssets(items, assets, captions);
    ...
    // attribute test
    bool AttributeTest(Graphics::Base &graphics, Graphics::Scenery scenes, Character::Base &character, Attribute::Type attribute, Item::Type weapon = Item::NONE, int add_roll = 0, int modifier = 0)
    {
        auto score = Engine::Score(character, attribute, false, weapon);
    ...
    void Fight(Graphics::Base &graphics, Graphics::Scenery scenes, Character::Base &attacker, Character::Base &defender, Skills::Type skill, Item::Property weapon)
    {
        ...
        auto knockout = (skill == Skills::Map("QUARTERSTAFF")) && attacker.HasSkill(Skills::Map("QUARTERSTAFF"));

        auto unarmed = (skill != Skills::Map("ARCHERY")) && attacker.IsPlayer() && !attacker.HasArmedWeapon();

        // defender endurance
        auto endurance = Engine::Score(defender, Attribute::Type::ENDURANCE, true, Item::NONE);

        // attacker score
        auto score = Engine::Score(attacker, Attribute::Type::FIGHTING_PROWESS, true, weapon);

        // Eye of the Tiger effects
        if (attacker.HasStatus(Character::Status::FPR_PLUS2))
        {
            score += 2;
        }
        else if (attacker.HasStatus(Character::Status::FPR_PLUS1))
        {
            score += 1;
        }
        ...
        roll += defender.IsStatus(Character::Status::DEFENDING) ? 1 : 0;

        roll += knockout ? 1 : 0;

        roll += attacker.HasStatus(Character::Status::NIGHTHOWL) ? 1 : 0;

        auto modifier = defender.HasSkill(Skills::Map("DODGING")) ? 1 : 0;

        auto prompt = (skill != Skills::NONE ? Skills::TypeMapping[skill] : std::string("FIGHT")) + std::string(": ") + std::to_string(roll) + std::string("D");
        ...
        while (!done)
        {
            ...
            // attacker
            scene.VerifyAndAdd(Scene::Element(Asset::Get(attacker.Asset), Point(box.X + BloodSwordRogue::HalfTile, box.Y + BloodSwordRogue::HalfTile)));

            // attack score
            Interface::RenderValue(scene, Interface::Numbers, asset, 2, score, box.X + BloodSwordRogue::HalfTile + BloodSwordRogue::ControlSpacing, box.Y + BloodSwordRogue::HalfTile);

            // defender
            scene.VerifyAndAdd(Scene::Element(Asset::Get(defender.Asset), Point(box.X + BloodSwordRogue::HalfTile + BloodSwordRogue::ControlSpacing * 5, box.Y + BloodSwordRogue::HalfTile)));

            // defender endurance
            Interface::RenderValue(scene, Interface::Numbers, Interface::AttributeAssets[Attribute::Type::ENDURANCE], 3, endurance, box.X + BloodSwordRogue::HalfTile + BloodSwordRogue::ControlSpacing * 6, box.Y + BloodSwordRogue::HalfTile);
            ...
            if (Input::Check(input))
            {
                ...
                else if (input.Type == Controls::MapType("CONFIRM"))
                {
                    if (rolls.Sum <= score)
                    {
                        ...
                        auto dmg_mod = Engine::Modifier(attacker, Attribute::Type::DAMAGE, true, weapon);
