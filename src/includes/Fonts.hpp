...
namespace BloodSwordRogue::Fonts
{
    ...
    // set up font types
    void Initialize(const char *font_ttf, int caption, int normal, int fixed, const char *zip_file)
    {
        TTF_Init();
    ...
    void Load(nlohmann::json &data, const char *zip_file)
    {
        ...
        {
            auto font = std::string(!data["font-settings"]["font"].is_null() ? data["font-settings"]["font"] : "modules/default/fonts/text.ttf");
            ...
            Fonts::Initialize(font.c_str(), caption, normal, fixed, zip_file);
        }
    }
    ...
    {
        auto ifs = zip_file != nullptr ? ZipFile::Read(zip_file, fonts) : Read(fonts);
    ...
    {
        Fonts::Load(fonts.c_str(), nullptr);
    }
